    ...
    const req = https.request(options, res => {
      ...
      res.on('end', () => {
        try {
          ...
          reject({ message: 'Parse error: ' + data.slice(0, 100) });
        }
      });
    });
...
client.on('messageCreate', async (message) => {
  ...
  try {
    const aiReply = await callGrok(getHistory(userId));
    ...
    else if (err.status === 401) return message.reply('XAI_API_KEY invalid.');
